...
def move_project_card_column(from_column_name, to_column_name, issue_number):
    project_id = int(get_project(ENV_VAR.config("PROJECT_BOARD_NAME"))["id"])
...
def process_issue(event_data):
    ...
    if "action" in event_data and event_data["action"] == "milestoned":
        move_project_card_column(from_column_name="Backlog", to_column_name="To Do", issue_number=issue_number)
